...
| Symptom | Resolution |
| --- | --- |
| `ImportError: MinkowskiEngine` | Install a wheel that matches your CUDA toolkit (`pip install -U "git+https://github.com/NVIDIA/MinkowskiEngine@v0.6.1"`) or build from source. CPU-only execution is limited. |
| `HTTPError` during fetch or SIFTS steps | Increase `--timeout`, reduce `--max-workers`, or re-run — downloads are retried and the pipeline is idempotent. |
| `ValueError: Dataset is empty` | Ensure `data/processed/*.h5` exists and that the `--ids-file` (if provided) lists valid identifiers. |
| GPU out-of-memory | Lower `--batch-size`, increase `--voxel-size`, or disable AMP. Both training scripts expose `--grad-max-norm` for stable accumulation. |

Run `python -m procore_seg.main_cli doctor` whenever your environment changes to confirm all essentials are installed.
...
1. Format code with `ruff`/`black`-compatible styles (PEP 8, type hints).
2. Include CLI examples and metadata updates in documentation as needed.
3. Run smoke tests (`SparseSegmentationUNet.smoke_test()`, dataset loaders) before submitting.

Please open a discussion before introducing new external dependencies or altering on-disk dataset formats.

---

## Citation

If you build upon ProCore-Seg in academic work, cite the repository and describe the voxel size, training hyperparameters, and evaluation protocol you used so reviewers can reconstruct the pipeline using the commands above.
